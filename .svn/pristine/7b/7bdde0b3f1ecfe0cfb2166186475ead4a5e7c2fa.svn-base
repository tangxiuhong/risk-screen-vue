            ...
            <el-col :span="12">
              <table class="tab-hover">
                ...
                <tr v-for="(item,index) in areaDataList.slice(0, 7)">
                  ...
                  <td @click="getSbHiddenUnitInfo(0,item.areaCode)" style="cursor:pointer;">{{item.SbUnitNum}}</td>
                  <td @click="getOverdueHiddenUnitInfo(0,item.areaCode)" style="cursor:pointer;">
            ...
            <el-col :span="12">
              <table class="tab-hover">
                ...
                <tr v-for="(item,index) in industryDataList.slice(0, 10)">
                  ...
                  <td @click="getSbHiddenUnitInfo(1,item.industryCode)" style="cursor:pointer;">{{item.BSbUnitNum}}</td>
                  <td @click="getOverdueHiddenUnitInfo(1,item.industryCode)" style="cursor:pointer;">
  ...
  export default {
    ...
    methods: {
      init () {
        ...
        this.getUnitTroubleAreaData()
        this.getUnitTroubleBusinessType()
      },
      // 13区街重大隐患情况
      getUnitTroubleAreaData () {
        this.$http({
          url: this.$http.adornUrl('/hiddenCheck/getUnitTroubleAreaData'),
          method: 'get',
      ...
      // 20个专委会隐患情况
      getUnitTroubleBusinessType () {
        this.$http({
          url: this.$http.adornUrl('/hiddenCheck/getUnitTroubleBusinessType'),
          method: 'get',
          params: this.$http.adornParams()
        }).then(({data}) => {
          if (data && data.code === 0) {
      ...
      // 申报一般隐患企业
      getSbHiddenUnitInfo(flag, code) {
        console.log(code)
        this.unitInfoVisible = true
        this.$nextTick(() => {
          this.$refs.unitInfo.init(8, flag, code)
        })
      },
      // 逾期未整改一般隐患企业
      getOverdueHiddenUnitInfo(flag, code) {
        console.log(code)
        this.unitInfoVisible = true
        this.$nextTick(() => {
          this.$refs.unitInfo.init(9, flag, code)
        })
      }
    }
  }
